...
const ProductSection = () => (
  <section id="products" className="py-24 lg:py-32 bg-white relative overflow-hidden">
    {/* Background Elements */}
    <div className="absolute top-0 left-0 w-96 h-96 bg-orange-100/50 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" />
    ...
    <div className="max-w-7xl mx-auto px-6 relative">
      ...
      >
        <span className="text-orange-600 font-semibold tracking-widest uppercase text-sm">What We Offer</span>
        <h2 className="text-4xl lg:text-5xl font-bold text-slate-900 mt-3">
          Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-amber-500">Expertise</span>
        </h2>
      ...
      >
        {products.map((item, idx) => (
          <motion.div 
            ...
            className="group relative bg-slate-50 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-500"
          >
            {/* Image */}
            ...
            {/* Content */}
            <div className="p-6">
              <h3 className="text-xl font-bold text-slate-900 group-hover:text-orange-600 transition-colors flex items-center justify-between">
                {item.title}
                <ArrowUpRight size={20} className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-orange-600" />
              </h3>
              ...
              <div className="mt-6 h-1 bg-slate-200 rounded-full overflow-hidden">
                <motion.div 
                  className="h-full bg-gradient-to-r from-orange-500 to-amber-500 w-0 group-hover:w-full transition-all duration-500"
                />
              </div>
            </div>
          </motion.div>
